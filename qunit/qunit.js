...
Test.prototype = {
	...
	setup: function() {
		...
		try {
			...
			QUnit.pushFailure( "初始化测试用例 " + this.testName + " 失败: " + e.message, extractStacktrace( e, 1 ) );
		}
	},
	run: function() {
		config.current = this;
		...
		try {
			...
			QUnit.pushFailure( "测试用例 #" + (this.assertions.length + 1) + "抛出了异常: " + e.message, extractStacktrace( e, 1 ) );
	...
	teardown: function() {
		...
		if ( config.notrycatch ) {
			...
			try {
				...
				QUnit.pushFailure( "无法执行测试用例 " + this.testName + " 的清理: " + e.message, extractStacktrace( e, 1 ) );
			}
		}
		checkPollution();
	},
	finish: function() {
		...
		if ( this.expected != null && this.expected != this.assertions.length ) {
			QUnit.pushFailure( "应该有 " + this.expected + " 个确认, 但只有 " + this.assertions.length + " 个确认被成功执行。" );
		} else if ( this.expected == null && !this.assertions.length ) {
			QUnit.pushFailure( "应该有至少一个确认, 但没有一个确认被执行 - 调用 expect(0) 来忽略本警告。" );
		}
...
extend(QUnit, {
	...
	push: function(result, actual, expected, message) {
		...
		message = escapeInnerText(message) || (result ? "通过" : "未通过");
		...
		if (!result) {
			...
			if (source) {
				details.source = source;
				output += '<tr class="test-source"><th>调用堆栈: </th><td><pre>' + escapeInnerText(source) + '</pre></td></tr>';
			}
			output += "</table>";
		}
